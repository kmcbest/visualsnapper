...
from PyQt6.QtGui import (QPixmap, QIcon, QAction, QWheelEvent, QColor, QBrush, 
                         QFileSystemModel, QImageReader, QPainter)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.dir_model.setRootPath('') # 允许访问所有驱动器
    ...
    def restore_last_path(self):
        """恢复上次打开的目录"""
        last_path = self.settings.value("last_directory", "")
        if last_path and os.path.isdir(last_path):
            # 展开到该路径
            index = self.dir_model.index(last_path)
            if index.isValid():
                self.tree.setCurrentIndex(index)
                self.tree.scrollTo(index)
                self.load_thumbnails(last_path)
